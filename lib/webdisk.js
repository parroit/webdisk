...
function dirExists(d, cb) {
    fs.stat(d, function(err, stat) {
        ...
        cb(err, stat ? stat.isDirectory() : false);
    });
}


exports.writeFile = function(filePath, stream) {
    var fullPath = path.resolve(root, filePath),
    ...
    return new Promise(function(resolve, reject) {
        ...
        dirExists(directory, function(err,exists) {
            if (!exists) {
                return resolve({
                    ok: false,
                    reason: "directory " + path.dirname(filePath) + " not found"
                });
            }
